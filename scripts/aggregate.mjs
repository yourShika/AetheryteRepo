...
async function main() {
  ...
  await writeJson(P("docs", "plugins.json"), {
    ...
    plugins: all.map((p) => ({
      ...
      testingVersion: p.TestingAssemblyVersion || "",
      apiLevel: p.DalamudApiLevel ?? null,
      testingApiLevel: p.TestingDalamudApiLevel ?? null,
      applicableVersion: p.ApplicableVersion || "any",
      lastUpdate: p.LastUpdate || 0,
      repoUrl: p.RepoUrl || "",
      iconUrl: p.IconUrl || "",
      downloadLink: p.DownloadLinkInstall || "",
      downloads: p.DownloadCount || 0,
      category: p._Category,
      source: p._SourceName,
      sourceUrl: p._SourceUrl,
